...
public sealed class DynamicMeterTests
{
    ...
    public void ReorderedProviderArrayKeepsMeterIdentity()
    {
        ...
        var extractor = new DynamicMeterExtractor();
        var a = extractor
            .Extract(Provider, first.RootElement, "fixture", Now, true)
            .ToDictionary(item => item.DisplayName);
        var b = extractor
            .Extract(Provider, second.RootElement, "fixture", Now, true)
            .ToDictionary(item => item.DisplayName);
    ...
    public void WindowEndSnakeCaseIsRecognizedAsResetTime()
    {
        ...
        var meter = Assert.Single(
            new DynamicMeterExtractor().Extract(Provider, document.RootElement, "fixture", Now, true)
        );
    ...
    {
        var previous = Snapshot(SnapshotCompleteness.Authoritative, Meter("short", 30), Meter("weekly", 40));
        var incoming = Snapshot(SnapshotCompleteness.Partial, Meter("weekly", 45)) with
        {
    ...
    {
        var previous = Snapshot(SnapshotCompleteness.Authoritative, Meter("short", 30), Meter("weekly", 40));
        var incoming = Snapshot(SnapshotCompleteness.Authoritative, Meter("weekly", 45)) with
        {
            ObservedAt = Now.AddMinutes(5),
        };
        var merged = new SnapshotMerger().Merge(previous, incoming);
    ...
    public void FactoryPoolsStayGroupedInWindowOrder()
    {
        ...
        Assert.Equal(
            ["five-hour", "weekly", "monthly", "core-five-hour", "core-weekly", "core-monthly"],
            OrderedKeys(meters)
        );
    }
    ...
    private static string[] OrderedKeys(IEnumerable<UsageMeter> meters) =>
        meters.OrderBy(meter => meter, MeterDisplayOrderComparer.Instance).Select(meter => meter.Key.Value).ToArray();
    ...
    private static UsageMeter DisplayMeter(string key, string name, double percent, DateTimeOffset reset) =>
        new(
            ...
            null,
            MeterStatus.Healthy,
            new MeterProvenance("fixture", "$", Now, true)
        );
