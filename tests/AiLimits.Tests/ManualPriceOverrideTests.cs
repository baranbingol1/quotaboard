...
public sealed class ManualPriceOverrideTests
{
    ...
    public void Quote_prices_each_lane_per_million()
    {
        var price = new ManualModelPrice(InputPerMillion: 3.0m, OutputPerMillion: 15.0m,
            CacheReadPerMillion: 0.3m, CacheWritePerMillion: 3.75m);

        decimal quote = price.QuoteUsd(inputTokens: 1_000_000, outputTokens: 2_000_000,
    ...
    public void Missing_cache_read_rate_prices_that_lane_at_zero()
    {
        ...
        decimal quote = price.QuoteUsd(inputTokens: 1_000_000, outputTokens: 0,
            cacheReadTokens: 5_000_000, cacheWriteTokens: 0);
    ...
    public void Missing_cache_write_rate_falls_back_to_the_input_rate()
    {
        ...
        decimal quote = price.QuoteUsd(inputTokens: 0, outputTokens: 0,
            cacheReadTokens: 0, cacheWriteTokens: 2_000_000);
    ...
    {
        var price = new ManualModelPrice(InputPerMillion: 3.0m, OutputPerMillion: 15.0m,
            CacheReadPerMillion: null, CacheWritePerMillion: 0m);
        ...
        var reloaded = ManualPriceOverrideSet.Parse(ManualPriceOverrideSet.Empty
            .With("amp", "model", price).Serialize());
    ...
    public void Reasoning_tokens_fold_into_the_output_lane()
    {
        ...
        decimal quote = price.QuoteUsd(inputTokens: 0, outputTokens: 1_000_000,
            cacheReadTokens: 0, cacheWriteTokens: 0, reasoningTokens: 1_000_000);
    ...
    {
        var set = ManualPriceOverrideSet.Empty
            .With("cursor", "composer-2.5-fast", new ManualModelPrice(1.5m, 6m, 0.15m))
    ...
    {
        var set = ManualPriceOverrideSet.Empty
            .With("cursor", "model", new ManualModelPrice(3m, 15m))
            .With("cursor", "model", null);

        Assert.False(set.TryGet("cursor", "model", out _));

        var zeroed = ManualPriceOverrideSet.Empty
            .With("cursor", "model", new ManualModelPrice(3m, 15m))
