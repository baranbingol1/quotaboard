...
public sealed class AccountHealthPolicyTests
{
    ...
    {
        Assert.Equal(AccountHealth.Live,
    ...
    public void A_fresh_snapshot_with_a_failed_latest_attempt_is_not_live()
    {
        ...
        Assert.Equal(AccountHealth.Offline,
            AccountHealthPolicy.Decide(isConnected: true, snapshotAge: TimeSpan.FromSeconds(30), FetchFailureKind.Network));
        Assert.Equal(AccountHealth.SignInRequired,
            AccountHealthPolicy.Decide(isConnected: true, snapshotAge: TimeSpan.FromSeconds(30), FetchFailureKind.Authentication));
    }

    [Fact]
    public void An_aged_snapshot_with_a_succeeded_latest_attempt_is_cached()
    {
        Assert.Equal(AccountHealth.Cached,
    ...
    {
        Assert.Equal(AccountHealth.Retrying,
            AccountHealthPolicy.Decide(isConnected: true, snapshotAge: null, FetchFailureKind.TemporarilyUnavailable));
        Assert.Equal(AccountHealth.Retrying,
            AccountHealthPolicy.Decide(isConnected: true, snapshotAge: TimeSpan.FromMinutes(10), FetchFailureKind.TemporarilyUnavailable));
    ...
    public void Auth_failures_require_sign_in_with_or_without_a_snapshot()
    {
        ...
        Assert.Equal(AccountHealth.SignInRequired,
            AccountHealthPolicy.Decide(isConnected: true, snapshotAge: TimeSpan.FromHours(1), FetchFailureKind.Authentication));
    }

    [Fact]
    public void A_disconnected_account_without_data_is_not_connected()
    {
        Assert.Equal(AccountHealth.NotConnected,
    ...
    {
        Assert.Equal(AccountHealth.NoQuota,
