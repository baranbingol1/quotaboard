...
public sealed class ProviderHttpTests
{
    ...
    private static Task<ProviderJsonResult> SendAsync(
    ...
    )
    {
        ...
        return ProviderHttp.GetJsonAsync(
            client,
            request,
            "test.strategy",
            "TestProvider",
    ...
    public async Task StatusCodesClassifyConsistently(
        HttpStatusCode status,
        FetchFailureKind expectedKind,
        FallbackPolicy expectedFallback
    )
    {
    ...
    public async Task InternalTimeoutClassifiesAsTimeoutNotCancellation()
    {
        ...
        using var result = await SendAsync(
            handler,
            options: new ProviderHttpOptions(Timeout: TimeSpan.FromMilliseconds(50))
        );
    ...
    public async Task CallerCancellationPropagatesAsException()
    {
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => SendAsync(handler, cancelled.Token));
    }
