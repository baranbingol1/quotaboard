...
public sealed class MeterDisplayOrderComparer : IComparer<UsageMeter>
{
    ...
    private static readonly string[] GenericPhrases = ["limit", "budget", "quota", "allowance", "usage"];
    ...
    public int Compare(UsageMeter? x, UsageMeter? y)
    {
        ...
        return comparison != 0 ? comparison : StringComparer.Ordinal.Compare(x.Key.Value, y.Key.Value);
    }
    ...
    private static int WindowOrder(UsageMeter meter)
    {
        if (meter.WindowDuration is { } duration)
        {
            if (duration <= TimeSpan.FromHours(8))
                return 0;
            if (duration <= TimeSpan.FromHours(36))
                return 1;
        ...
        if (
            Contains(name, "5-hour")
            || Contains(name, "5 hour")
            || Contains(name, "five-hour")
            || Contains(name, "hourly")
            || Contains(name, "session")
        )
            return 0;
        if (Contains(name, "daily"))
            return 1;
        if (Contains(name, "weekly"))
            return 2;
        if (Contains(name, "monthly"))
            return 3;
        return 4;
    }
