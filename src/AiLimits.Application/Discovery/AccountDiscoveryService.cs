...
public sealed class AccountDiscoveryService
{
    ...
    public AccountDiscoveryService(IReadOnlyList<IProviderAdapter> adapters, IAccountRepository accounts, ILogger<AccountDiscoveryService> logger)
    {
    ...
    public async Task<IReadOnlyList<ProviderAccount>> DiscoverAsync(CancellationToken cancellationToken)
    {
        ...
        Dictionary<AccountKey, ProviderAccount> existingByKey = existing.ToDictionary((ProviderAccount account) => account.Key);
        ...
        foreach (IProviderAdapter adapter in _adapters)
        {
            ...
            {
                _logger.LogWarning(exception, "Account discovery failed for provider {Provider}; saved accounts are preserved.", adapter.Descriptor.Id.Value);
                continue;
            }
        ...
        foreach (ProviderAccount vanished in existing.Where((ProviderAccount account) => account.IsConnected
            && succeededProviders.Contains(account.Key.Provider)
            && !discoveredKeys.Contains(account.Key)))
        {
            await _accounts.UpsertAsync(vanished with
            {
                IsConnected = false,
    ...
    private static ProviderAccount MergeAccount(ProviderAccount? existing, ProviderAccount discovered)
    {
        ...
        bool authChanged = !string.Equals(existing.AuthSource, discovered.AuthSource, StringComparison.Ordinal) || !string.Equals(existing.Login, discovered.Login, StringComparison.Ordinal);
        return discovered with
        {
            ConfigurationRevision = (authChanged ? (existing.ConfigurationRevision + 1) : existing.ConfigurationRevision),
