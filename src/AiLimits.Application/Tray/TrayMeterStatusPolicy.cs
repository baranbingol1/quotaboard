...
{
    public static bool IsCurrent(MeterStatus status) => status is
        MeterStatus.Healthy
        or MeterStatus.Approaching
        or MeterStatus.Critical
        or MeterStatus.Exhausted;

    public static int Severity(MeterStatus status) => status switch
    {
    ...
    public static MeterStatus Worst(IEnumerable<MeterStatus> statuses) =>
        statuses
            ...
            .FirstOrDefault(MeterStatus.Healthy);
}
