...
{
    private static readonly Uri ResetCreditsUri = new Uri("https://chatgpt.com/backend-api/wham/rate-limit-reset-credits");
    ...
    {
        CliCredential credential = await CliCredentialReader.ReadCodexAsync(Path.Combine(_codexHome, "auth.json"), cancellationToken).ConfigureAwait(false);
        IReadOnlyList<ProviderAccount> result;
        if (credential is not null)
        {
            IReadOnlyList<ProviderAccount> readOnlyList = new ProviderAccount[] { new ProviderAccount(new AccountKey(Descriptor.Id, credential.AccountId), credential.Login ?? "Codex CLI account", credential.Login, "Codex CLI OAuth", 1L, IsConnected: true) };
            result = readOnlyList;
        }
        else
        {
    ...
    {
        return new ILimitFetchStrategy[] { new CodexOAuthLimitStrategy(_httpClient, _clock, account, Path.Combine(_codexHome, "auth.json")) };
    }
    ...
    public async Task<ResetCreditInventory?> FetchResetCreditsAsync(ProviderAccount account, CancellationToken cancellationToken)
    {
        CliCredential credential = await CliCredentialReader.ReadCodexAsync(Path.Combine(_codexHome, "auth.json"), cancellationToken).ConfigureAwait(false);
    ...
    {
        if (!root.TryGetProperty("credits", out JsonElement creditsElement) || creditsElement.ValueKind != JsonValueKind.Array)
        {
            return null;
        }
