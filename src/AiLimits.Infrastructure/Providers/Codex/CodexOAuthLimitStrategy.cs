...
internal sealed class CodexOAuthLimitStrategy(HttpClient httpClient, IClock clock, ProviderAccount expectedAccount, string credentialPath) : ILimitFetchStrategy
{
    private static readonly Uri UsageUri = new Uri("https://chatgpt.com/backend-api/wham/usage");

    private static readonly IReadOnlyDictionary<string, MeterAlias> Aliases = new Dictionary<string, MeterAlias>(StringComparer.OrdinalIgnoreCase)
    {
    ...
    public async Task<StrategyAvailabilityResult> CheckAvailabilityAsync(ProviderAccount account, CancellationToken cancellationToken)
    {
        return await CliCredentialReader.ReadCodexAsync(credentialPath, cancellationToken).ConfigureAwait(false) is null
            ? new StrategyAvailabilityResult(StrategyAvailability.NotConfigured, "Codex CLI OAuth credentials were not found.")
    ...
    public async Task<FetchResult> FetchAsync(ProviderAccount account, CancellationToken cancellationToken)
    {
        ...
        CliCredential credential = await CliCredentialReader.ReadCodexAsync(credentialPath, cancellationToken).ConfigureAwait(false);
        if (credential is null)
        {
            return FetchResult.Failure(FetchFailureKind.Authentication, "Codex CLI OAuth credentials are unavailable.", FallbackPolicy.TryNextStrategy, Id, Stopwatch.GetElapsedTime(started));
        }
        if (!string.Equals(credential.AccountId, expectedAccount.Key.Value, StringComparison.Ordinal))
        {
            return FetchResult.Failure(FetchFailureKind.AccountMismatch, "Codex credentials now belong to a different account.", FallbackPolicy.Stop, Id, Stopwatch.GetElapsedTime(started));
        }
        ...
        using ProviderJsonResult exchange = await ProviderHttp.GetJsonAsync(httpClient, request, Id, "Codex", started, cancellationToken).ConfigureAwait(false);
        ...
        if (meters.Count == 0)
        {
            return FetchResult.Failure(FetchFailureKind.ProviderChanged, "Codex returned no recognizable limit meters.", FallbackPolicy.TryNextStrategy, Id, Stopwatch.GetElapsedTime(started));
        }
        string? planType = document.RootElement.TryGetProperty("plan_type", out var planElement) && planElement.ValueKind == JsonValueKind.String
        ...
        ProviderSnapshot snapshot = new ProviderSnapshot(expectedAccount.Key, meters, ExtractBalances(document.RootElement), SnapshotCompleteness.Authoritative, now, DataConfidence.High, extensions);
    ...
    internal IReadOnlyList<UsageMeter> ExtractMeters(JsonElement root, ProviderId provider, DateTimeOffset now)
    {
        ...
        {
            AddWindowMeter(meters, usedNames, rateLimit, "primary_window", "$.rate_limit.primary_window", "Primary limit", "codex:window:primary", MeterScope.Account, null, provider, now);
            AddWindowMeter(meters, usedNames, rateLimit, "secondary_window", "$.rate_limit.secondary_window", "Secondary limit", "codex:window:secondary", MeterScope.Account, null, provider, now);
        }
        if (root.TryGetProperty("additional_rate_limits", out JsonElement additional) && additional.ValueKind == JsonValueKind.Array)
        {
            int index = 0;
        ...
        foreach (UsageMeter meter in new DynamicMeterExtractor().Extract(provider, root, Id, now, authoritative: true, Aliases))
        {
    ...
    private void AddWindowMeter(List<UsageMeter> meters, HashSet<string> usedNames, JsonElement parent, string property, string path, string fallbackName, string key, MeterScope scope, string? namePrefix, ProviderId provider, DateTimeOffset now)
    {
        ...
        meters.Add(new UsageMeter(new MeterKey(key), label, scope, MeterUnit.Percent, null, null, Math.Clamp(usedPercent, 0.0, 100.0), duration, resetsAt, null, StatusFromPercent(usedPercent), new MeterProvenance(Id, path, now, IsAuthoritative: true)));
    }

    private void AddAdditionalLimitMeters(List<UsageMeter> meters, HashSet<string> usedNames, JsonElement entry, int index, ProviderId provider, DateTimeOffset now)
    {
        if (entry.ValueKind != JsonValueKind.Object || !entry.TryGetProperty("rate_limit", out JsonElement rateLimit) || rateLimit.ValueKind != JsonValueKind.Object)
        {
            return;
        }
        ...
        AddWindowMeter(meters, usedNames, rateLimit, "primary_window", basePath + ".primary_window", display + " limit", $"codex:extra:{slug}:primary", MeterScope.Model, display, provider, now);
        AddWindowMeter(meters, usedNames, rateLimit, "secondary_window", basePath + ".secondary_window", display + " limit", $"codex:extra:{slug}:secondary", MeterScope.Model, display, provider, now);
    }
    ...
    private static bool TryReadWindow(JsonElement window, out double usedPercent, out DateTimeOffset? resetsAt, out TimeSpan? duration)
    {
        usedPercent = 0.0;
        resetsAt = null;
        duration = null;
        ...
        if (window.TryGetProperty("used_percent", out JsonElement percentElement))
        {
            if (percentElement.ValueKind == JsonValueKind.Number && percentElement.TryGetDouble(out double numeric))
            {
                ...
                && double.TryParse(percentElement.GetString(), System.Globalization.NumberStyles.Number, System.Globalization.CultureInfo.InvariantCulture, out double parsed))
            {
                usedPercent = parsed;
                hasPercent = true;
            }
        ...
        foreach (string resetField in new[] { "reset_at", "resets_at" })
        {
            ...
            {
                resetsAt = unix > 10000000000L ? DateTimeOffset.FromUnixTimeMilliseconds(unix) : DateTimeOffset.FromUnixTimeSeconds(unix);
                break;
            }
        ...
        if (window.TryGetProperty("limit_window_seconds", out JsonElement secondsElement) && secondsElement.ValueKind == JsonValueKind.Number && secondsElement.TryGetDouble(out double seconds) && seconds > 0)
        {
            duration = TimeSpan.FromSeconds(seconds);
        }
        else if (window.TryGetProperty("limit_window_minutes", out JsonElement minutesElement) && minutesElement.ValueKind == JsonValueKind.Number && minutesElement.TryGetDouble(out double minutes) && minutes > 0)
        {
    ...
    {
        return element.TryGetProperty(property, out JsonElement value) && value.ValueKind == JsonValueKind.String && !string.IsNullOrWhiteSpace(value.GetString())
    ...
    private static IReadOnlyList<BalanceMetric> ExtractBalances(JsonElement root)
    {
        ...
        foreach (string propertyName in array)
        {
            ...
            if (value2.ValueKind == JsonValueKind.String
                && decimal.TryParse(value2.GetString(), System.Globalization.NumberStyles.Number, System.Globalization.CultureInfo.InvariantCulture, out var parsed))
            {
