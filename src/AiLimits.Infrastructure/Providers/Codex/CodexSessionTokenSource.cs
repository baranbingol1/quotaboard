...
public sealed class CodexSessionTokenSource(string codexHome) : ITokenUsageSource
{
            ...
            while (await reader.ReadLineAsync(cancellationToken).ConfigureAwait(false) is { } line)
            {
                ...
                using (document)
                {
                    ...
                    if (!TryTimestamp(root, out var occurredAt))
                        continue;
                    if (
                        payload.TryGetProperty("model", out var modelNode)
                    ...
                    var info =
                        payload.TryGetProperty("info", out var infoNode) && infoNode.ValueKind == JsonValueKind.Object
                            ? infoNode
                            : payload;
                    if (
                    ...
                    if (input + output + delta.Cache + delta.Reasoning == 0)
                        continue;
                    ...
                    yield return new TokenUsageEvent(
                        ...
                        0,
                        delta.Reasoning,
                        $"codex:{Path.GetFileName(file)}:{lineNumber}",
                        project
                    );
                }
            }
    ...
    {
        if (ReadString(payload, "cwd") is { } payloadCwd)
            return payloadCwd;
        if (
            payload.TryGetProperty("turn_context", out var turnContext)
            && turnContext.ValueKind == JsonValueKind.Object
            && ReadString(turnContext, "cwd") is { } turnCwd
        )
            return turnCwd;
    ...
    private readonly record struct Counts(long Input, long Output, long Cache, long Reasoning)
    {
        ...
        public Counts DeltaFrom(Counts previous) =>
            new(
                ...
                Delta(Cache, previous.Cache),
                Delta(Reasoning, previous.Reasoning)
            );
