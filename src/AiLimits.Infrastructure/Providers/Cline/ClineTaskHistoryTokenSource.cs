...
public sealed class ClineTaskHistoryTokenSource(IReadOnlyList<string> roots) : ITokenUsageSource
{
        ...
        foreach (string root in roots)
        {
            ...
            if (Directory.Exists(tasksRoot))
            {
                             ...
                             .OrderBy(path => path, StringComparer.OrdinalIgnoreCase))
                {
                    ...
                    await foreach (TokenUsageEvent usage in ScanTaskFile(
                                       file, taskId, info?.ModelId, project, account, cursor, cancellationToken)
            ...
            if (Directory.Exists(sessionsRoot))
            {
                             ...
                             .OrderBy(path => path, StringComparer.OrdinalIgnoreCase))
                {
                    ...
                    await foreach (TokenUsageEvent usage in ScanSessionFile(
                                       file, sessionId, project, account, cursor, cancellationToken)
                                   .ConfigureAwait(false))
                    {
                        yield return usage;
                    }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        using (document)
        {
            ...
            foreach (JsonElement message in document.RootElement.EnumerateArray())
            {
                ...
                if (!message.TryGetProperty("say", out JsonElement say) ||
                    say.ValueKind != JsonValueKind.String ||
                    !string.Equals(say.GetString(), "api_req_started", StringComparison.Ordinal)) continue;
                if (!message.TryGetProperty("ts", out JsonElement tsNode) || !TryReadInt64(tsNode, out long ts)) continue;
                if (!message.TryGetProperty("text", out JsonElement textNode) ||
                    textNode.ValueKind != JsonValueKind.String ||
                    string.IsNullOrWhiteSpace(textNode.GetString())) continue;

                Counts current;
                JsonDocument? payload;
                try { payload = JsonDocument.Parse(textNode.GetString()!); }
                catch (JsonException) { continue; }
                using (payload)
                {
                    if (payload.RootElement.ValueKind != JsonValueKind.Object) continue;
                    current = new Counts(
                ...
                string? backend = null;
                string? model = null;
                if (message.TryGetProperty("modelInfo", out JsonElement modelInfo) &&
                ...
                yield return new TokenUsageEvent(
                    account.Key, new ServiceProviderId("cline"), ComposeRawModelId(model, backend), occurredAt,
                    delta.Input, delta.Output, delta.CacheRead, delta.CacheWrite, 0,
                    $"cline:task:{taskId}:{ts.ToString(CultureInfo.InvariantCulture)}", project);
            }
        }
    }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        using (document)
        {
            ...
            if (!document.RootElement.TryGetProperty("messages", out JsonElement messages) ||
                messages.ValueKind != JsonValueKind.Array) yield break;
            Dictionary<string, Counts> seen = new(StringComparer.Ordinal);
            foreach (JsonElement message in messages.EnumerateArray())
            {
                ...
                if (!message.TryGetProperty("role", out JsonElement role) ||
                    ...
                    !string.Equals(role.GetString(), "assistant", StringComparison.Ordinal)) continue;
                if (!message.TryGetProperty("metrics", out JsonElement metrics) ||
                    metrics.ValueKind != JsonValueKind.Object) continue;
                if (!message.TryGetProperty("ts", out JsonElement tsNode) || !TryReadInt64(tsNode, out long ts)) continue;
                ...
                yield return new TokenUsageEvent(
                    account.Key, new ServiceProviderId("cline"), ComposeRawModelId(model, backend), occurredAt,
                    delta.Input, delta.Output, delta.CacheRead, delta.CacheWrite, 0,
                    $"cline:session:{sessionId}:{messageKey}", project);
            }
        }
    }
    ...
    private static IReadOnlyDictionary<string, ClineTaskInfo> LoadTaskHistory(string root)
    {
        ...
        if (!File.Exists(file)) return tasks;
        try
        {
            using FileStream stream = new(file, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
            using JsonDocument document = JsonDocument.Parse(stream);
            if (document.RootElement.ValueKind != JsonValueKind.Array) return tasks;
    ...
    private ProjectIdentity ResolveSessionProject(string sessionDirectory, string sessionId)
    {
        ...
        if (File.Exists(file))
        {
            try
            {
                ...
                {
                    string? cwd = ReadString(document.RootElement, "cwd") ??
                                  ReadString(document.RootElement, "workspace_root");
                    if (cwd is not null)
                    {
