...
internal sealed class DroidApiLimitStrategy(HttpClient httpClient, IClock clock, string? strategyId = null, int strategyOrder = 20) : ILimitFetchStrategy
{
    private static readonly Uri LimitsUri = new Uri("https://api.factory.ai/api/billing/limits");

    private static readonly Uri UsageUri = new Uri("https://api.factory.ai/api/organization/subscription/usage?useCache=true");
    ...
    {
        return Task.FromResult(string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("FACTORY_API_KEY")) ? new StrategyAvailabilityResult(StrategyAvailability.NotConfigured, "Optional Factory API key is not configured.") : StrategyAvailabilityResult.Ready());
    }
    ...
    public async Task<FetchResult> FetchAsync(ProviderAccount account, CancellationToken cancellationToken)
    {
        ...
        {
            return FetchResult.Failure(FetchFailureKind.Authentication, "Optional Factory API key is not configured.", FallbackPolicy.TryNextStrategy, Id, Stopwatch.GetElapsedTime(started));
        }
        return await FetchWithTokenAsync(account, token, cancellationToken).ConfigureAwait(false);
    }

    internal async Task<FetchResult> FetchWithTokenAsync(ProviderAccount account, string token, CancellationToken cancellationToken)
    {
        long started = Stopwatch.GetTimestamp();
        (string? PlanName, string? Email) accountInfo = await FetchAccountInfoAsync(token, cancellationToken).ConfigureAwait(false);
        using (ProviderJsonResult limits = await FetchJsonAsync(LimitsUri, token, started, cancellationToken).ConfigureAwait(false))
        {
            if (limits.IsSuccess)
            {
                FetchResult result = BuildSnapshot(account.Key, limits.Document!.RootElement, billingLimits: true, started, accountInfo.PlanName, accountInfo.Email);
                if (result.IsSuccess)
                {
                    return result;
                }
        ...
        using ProviderJsonResult usage = await FetchJsonAsync(UsageUri, token, started, cancellationToken).ConfigureAwait(false);
        if (!usage.IsSuccess)
        {
            return usage.Failure!;
        }
        return BuildSnapshot(account.Key, usage.Document!.RootElement, billingLimits: false, started, accountInfo.PlanName, accountInfo.Email);
    }
    ...
    private async Task<(string? PlanName, string? Email)> FetchAccountInfoAsync(string token, CancellationToken cancellationToken)
    {
        using ProviderJsonResult me = await FetchJsonAsync(new Uri("https://app.factory.ai/api/app/auth/me"), token, Stopwatch.GetTimestamp(), cancellationToken).ConfigureAwait(false);
        if (!me.IsSuccess)
        {
            return (null, null);
        }
        ...
        string? email = ReadNestedString(root, "userProfile", "email")
            ?? ReadNestedString(root, "userProfile", "workosUser", "email");
        return (planName, email);
    }

    internal FetchResult BuildSnapshot(AccountKey account, JsonElement root, bool billingLimits, long started, string? planName = null, string? email = null)
    {
        ...
        List<UsageMeter> list = new DynamicMeterExtractor().Extract(account.Provider, root, Id, utcNow, authoritative: true, aliases)
            .Select(meter => meter.Provenance.SourcePath.Contains(".core.", StringComparison.OrdinalIgnoreCase)
                ? meter with { DisplayName = "Droid Core " + char.ToLowerInvariant(meter.DisplayName[0]) + meter.DisplayName[1..] }
                : meter)
            ...
            .Select(meter => meter.ResetsAt.HasValue && meter.ResetsAt.Value <= utcNow
                ? meter with { Used = null, UsedPercent = 0.0, ResetsAt = null, WindowDuration = null, Status = MeterStatus.Healthy }
                : meter)
            .ToList();
        ...
        if (list.Count == 0)
        {
            return FetchResult.Failure(FetchFailureKind.ProviderChanged, "Factory returned no recognizable subscription limits.", FallbackPolicy.TryNextStrategy, Id, Stopwatch.GetElapsedTime(started));
        }
        ...
        if (!string.IsNullOrWhiteSpace(planName)) extensionValues.Add(("plan_type", planName.Trim()));
        ...
        return FetchResult.Success(new ProviderSnapshot(account, list, list2, SnapshotCompleteness.Authoritative, utcNow, DataConfidence.High, extensions), Id, Stopwatch.GetElapsedTime(started));
    }

    private void AddLegacyTokenMeters(AccountKey account, JsonElement usage, DateTimeOffset now, ICollection<UsageMeter> meters)
    {
        DateTimeOffset? resetsAt = null;
        if (usage.TryGetProperty("endDate", out var value) && value.TryGetInt64(out var value2))
        {
            resetsAt = ((value2 > 10000000000L) ? DateTimeOffset.FromUnixTimeMilliseconds(value2) : DateTimeOffset.FromUnixTimeSeconds(value2));
        }
        ...
        foreach (string text in array)
        {
            ...
            if (!(valueOrDefault == 0m) || !(valueOrDefault2 == 0m) || num.HasValue)
            {
                ...
                double num3 = num2;
                meters.Add(new UsageMeter(new MeterKey("droid:" + text), CultureInfo.InvariantCulture.TextInfo.ToTitleCase(text) + " tokens", MeterScope.Account, MeterUnit.Tokens, valueOrDefault, (valueOrDefault2 > 0m) ? valueOrDefault2 : null, Math.Clamp(num3, 0.0, 100.0), null, resetsAt, null, (num3 >= 95.0) ? MeterStatus.Critical : ((!(num3 >= 80.0)) ? MeterStatus.Healthy : MeterStatus.Approaching), new MeterProvenance(Id, "$.usage." + text, now, IsAuthoritative: true)));
            }
        }
    }
    ...
    private async Task<ProviderJsonResult> FetchJsonAsync(Uri uri, string token, long startedTimestamp, CancellationToken cancellationToken)
    {
        ...
        return await ProviderHttp.GetJsonAsync(httpClient, request, Id, "Factory", startedTimestamp, cancellationToken).ConfigureAwait(false);
    }
}
