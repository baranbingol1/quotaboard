...
internal sealed class DroidSessionLimitStrategy(
    HttpClient httpClient,
    IClock clock,
    FactoryCredentialReader credentialReader
) : ILimitFetchStrategy
{
    ...
    )
    {
        return await credentialReader.ReadAsync(cancellationToken).ConfigureAwait(false) is null
            ? new StrategyAvailabilityResult(
                ...
                "Sign in with the Droid CLI to connect Factory."
            )
    ...
    public async Task<FetchResult> FetchAsync(ProviderAccount account, CancellationToken cancellationToken)
    {
        ...
        if (credential is null)
        {
            return FetchResult.Failure(
                ...
                "Factory CLI session was not found. Run droid and sign in in the browser.",
                FallbackPolicy.TryNextStrategy,
                Id,
                Stopwatch.GetElapsedTime(started)
            );
        }
        return await new DroidApiLimitStrategy(httpClient, clock, Id, Order)
            .FetchWithTokenAsync(account, credential.AccessToken, cancellationToken)
