...
internal sealed class OpenCodeGoLocalLimitStrategy(OpenCodePathDiscovery discovery, IClock clock) : ILimitFetchStrategy
{
    ...
    {
        return File.Exists(await discovery.FindDatabaseAsync(cancellationToken).ConfigureAwait(false))
            ? StrategyAvailabilityResult.Ready()
            : new StrategyAvailabilityResult(StrategyAvailability.NotConfigured, "OpenCode database was not found.");
    }
    ...
    public async Task<FetchResult> FetchAsync(ProviderAccount account, CancellationToken cancellationToken)
    {
        ...
        if (!File.Exists(path))
        {
            return FetchResult.Failure(
                ...
                "OpenCode database was not found.",
        ...
        try
        {
            ...
            if (rows.Count == 0)
            {
                return FetchResult.Failure(
                    FetchFailureKind.Unsupported,
                    "No OpenCode Go subscription usage was found in the local database.",
                    FallbackPolicy.TryNextStrategy,
                    Id,
                    Stopwatch.GetElapsedTime(started)
                );
            ...
            if (
                !await HasOpenCodeAuthAsync(path, cancellationToken).ConfigureAwait(false)
            ...
            )
            {
                return FetchResult.Success(
                    new ProviderSnapshot(
                        ...
                        ProviderHttpSupport.SafeExtensions(("offering", "go-local-inactive"))
                    ),
                    Id,
                    Stopwatch.GetElapsedTime(started)
                );
            ...
            DateTimeOffset? oldestInSession = rows.Where((CostRow row) => row.At >= sessionStart && row.At < now)
                .MinBy((CostRow row) => row.At)
                ?.At;
            MeterProvenance provenance = new MeterProvenance(
                Id,
                "opencode.db:message/part.cost",
                now,
                IsAuthoritative: true
            );
            return FetchResult.Success(
                new ProviderSnapshot(
                    Meters: new UsageMeter[]
                    {
                        Meter(
                            "go-session-cost",
                            "Go session budget",
                    ...
                    Account: account.Key,
                    Balances: Array.Empty<BalanceMetric>(),
                    Completeness: SnapshotCompleteness.Authoritative,
                    ObservedAt: now,
                    Confidence: DataConfidence.Medium,
                    Extensions: ProviderHttpSupport.SafeExtensions(("offering", "go-local"))
                ),
                Id,
                Stopwatch.GetElapsedTime(started)
            );
            ...
            (num >= 100.0)
                ...
                : (
                    (num >= 95.0)
                        ...
                        : ((!(num >= 80.0)) ? MeterStatus.Healthy : MeterStatus.Approaching)
                )
        );
        return new UsageMeter(
            new MeterKey("opencode:" + key),
            name,
            MeterScope.Offering,
            MeterUnit.Usd,
            used,
    ...
    private static async Task<IReadOnlyList<CostRow>> ReadCostRowsAsync(
        string path,
        CancellationToken cancellationToken
    )
    {
                        ...
                        if (milliseconds > 0 && double.IsFinite(cost) && cost >= 0.0)
                        {
                            rows.Add(
                                new CostRow(
                                    ...
                                    Convert.ToDecimal(cost, CultureInfo.InvariantCulture)
                                )
                            );
                        }
                    }
    ...
    private static async Task<bool> HasOpenCodeAuthAsync(string databasePath, CancellationToken cancellationToken)
    {
        try
        {
            ...
            using System.Text.Json.JsonDocument document = await System
                .Text.Json.JsonDocument.ParseAsync(
                    stream,
                    default(System.Text.Json.JsonDocumentOptions),
                    cancellationToken
                )
                .ConfigureAwait(false);
            return document.RootElement.TryGetProperty("opencode", out _)
                || document.RootElement.TryGetProperty("opencode-go", out _)
                || document.RootElement.TryGetProperty("opencode-zen", out _);
        }
