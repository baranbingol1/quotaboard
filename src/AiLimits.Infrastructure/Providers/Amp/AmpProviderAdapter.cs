...
public sealed class AmpProviderAdapter(HttpClient http, IClock clock, IProcessRunner runner) : IProviderAdapter
{
    ...
    public Task<IReadOnlyList<ProviderAccount>> DiscoverAccountsAsync(CancellationToken cancellationToken)
    {
        ...
        [
            new(new AccountKey(Descriptor.Id, "default"), "Amp", null, cli ? "Amp CLI" : "Access token", 1, cli || api),
        ];
...
internal static class AmpParser
{
    ...
    private static readonly Regex Money = new(
        @"Amp Free:[ ]*[$](?<r>[0-9.]+)[ ]*/[ ]*[$](?<l>[0-9.]+)[ ]+remaining(?:[ ]+[(]replenishes[ ]+[+][$](?<h>[0-9.]+)[ ]*/hour[)])?",
        RegexOptions.IgnoreCase
    );
    private static readonly Regex Percent = new(@"Amp Free:[ ]*(?<p>[0-9.]+)%[ ]+remaining", RegexOptions.IgnoreCase);
    private static readonly Regex Individual = new(
        @"Individual credits?:[ ]*[$](?<v>[0-9.]+)",
        RegexOptions.IgnoreCase
    );
    ...
    private static readonly Regex Subscription = new(
        @"Subscription\s+(?<plan>[^:\r\n]+):(?<usage>[^\r\n]+)",
        RegexOptions.IgnoreCase
    );
    private static readonly Regex AgentUsage = new(
        @"(?<p>[0-9.]+)%\s+(?:other|agent)\s+usage",
        RegexOptions.IgnoreCase
    );
    ...
    internal static bool TryParse(string? raw, out AmpUsage? usage)
    {
        ...
        Match m = Money.Match(text),
            p = Percent.Match(text),
            i = Individual.Match(text),
            w = Workspace.Match(text),
            e = Identity.Match(text),
            s = Subscription.Match(text);
        ...
        usage = null;
        if (!m.Success && !p.Success && !i.Success && !w.Success && !a.Success && !o.Success)
            return false;
        usage = new(
            Dec(m, "r"),
            Dec(m, "l"),
    ...
    internal static ProviderSnapshot Snapshot(
        AccountKey account,
        AmpUsage value,
        DateTimeOffset now,
        string strategy,
    ...
    )
    {
        ...
        if (value.Individual.HasValue)
            balances.Add(new("amp:individual", "Individual credits", value.Individual, MeterUnit.Usd));
        if (value.Workspace.HasValue)
            balances.Add(new("amp:workspace", "Workspace credits", value.Workspace, MeterUnit.Usd));
        List<(string Key, string Value)> extensionValues = [("source", source)];
        if (!string.IsNullOrWhiteSpace(value.Email))
            extensionValues.Add(("email", value.Email));
        if (!string.IsNullOrWhiteSpace(value.SubscriptionPlan))
            extensionValues.Add(("plan_type", value.SubscriptionPlan));
    ...
    private static void AddSubscriptionMeter(
        List<UsageMeter> meters,
        string key,
        string name,
        double? remainingPercent,
    ...
    private static UsageMeter Meter(
        string key,
        string name,
        decimal used,
        decimal? limit,
    ...
    private static decimal? Dec(Match m, string g) =>
        m.Success
        && decimal.TryParse(m.Groups[g].Value, NumberStyles.Number, CultureInfo.InvariantCulture, out decimal v)
            ? v
            : null;

    private static double? Num(Match m, string g) =>
        m.Success && double.TryParse(m.Groups[g].Value, NumberStyles.Number, CultureInfo.InvariantCulture, out double v)
            ? v
            : null;
}
...
internal sealed class AmpCliStrategy(IProcessRunner runner, IClock clock) : ILimitFetchStrategy
{
    ...
    public Task<StrategyAvailabilityResult> CheckAvailabilityAsync(
        ...
        Task.FromResult(
            FindExecutable() is null
                ? new(StrategyAvailability.NotConfigured, "Amp CLI was not found.")
    ...
    public async Task<FetchResult> FetchAsync(ProviderAccount account, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ProcessResult result = await runner
                .RunAsync(executable, ["usage"], TimeSpan.FromSeconds(15), cancellationToken)
                .ConfigureAwait(false);
            if (result.OutputTruncated || result.ErrorTruncated)
            {
                return FetchResult.Failure(
                    FetchFailureKind.MalformedResponse,
                    "Amp usage output exceeded the capture limit and was truncated.",
                    FallbackPolicy.TryNextStrategy,
                    Id,
                    Stopwatch.GetElapsedTime(started)
                );
            ...
            return result.ExitCode == 0 && AmpParser.TryParse(output, out AmpUsage? usage)
                ? FetchResult.Success(
                    AmpParser.Snapshot(account.Key, usage, clock.UtcNow, Id, "cli"),
                    Id,
                    Stopwatch.GetElapsedTime(started)
                )
                : FetchResult.Failure(
                    FetchFailureKind.MalformedResponse,
                    "Amp returned no recognizable usage data.",
                    FallbackPolicy.TryNextStrategy,
                    Id,
                    Stopwatch.GetElapsedTime(started)
                );
    ...
    internal static string? FindExecutable()
    {
        ...
        foreach (
            string dir in (Environment.GetEnvironmentVariable("PATH") ?? "").Split(
                Path.PathSeparator,
        ...
        {
            string path = Path.Combine(dir.Trim('"'), name);
            if (File.Exists(path))
                return path;
        }
        return null;
...
internal sealed class AmpApiStrategy(HttpClient http, IClock clock) : ILimitFetchStrategy
{
    ...
    public Task<StrategyAvailabilityResult> CheckAvailabilityAsync(
        ...
        Task.FromResult(
            string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("AMP_API_KEY"))
                ? new(StrategyAvailability.NotConfigured, "AMP_API_KEY is not configured.")
                : StrategyAvailabilityResult.Ready()
        );
    ...
    public async Task<FetchResult> FetchAsync(ProviderAccount account, CancellationToken cancellationToken)
    {
        ...
        using ProviderJsonResult exchange = await ProviderHttp
            .GetJsonAsync(http, request, Id, "Amp", started, cancellationToken)
            .ConfigureAwait(false);
        ...
        bool found =
            json.RootElement.TryGetProperty("result", out JsonElement result)
            && result.TryGetProperty("displayText", out display);
        return found && AmpParser.TryParse(display.GetString(), out AmpUsage? usage)
            ? FetchResult.Success(
                AmpParser.Snapshot(account.Key, usage, clock.UtcNow, Id, "api"),
                Id,
                Stopwatch.GetElapsedTime(started)
            )
            : FetchResult.Failure(
                FetchFailureKind.MalformedResponse,
                "Amp returned no recognizable balance data.",
                FallbackPolicy.TryNextStrategy,
                Id,
                Stopwatch.GetElapsedTime(started)
            );
