...
public sealed class GitHubDeviceFlowClient(HttpClient httpClient, string clientId)
{
    ...
    public async Task<DeviceAuthorization> StartAsync(CancellationToken cancellationToken)
    {
        ...
        request.Content = new FormUrlEncodedContent(
            new Dictionary<string, string> { ["client_id"] = clientId, ["scope"] = "read:user read:org" }
        );
        ...
        if (
            !root.TryGetProperty("device_code", out var deviceCodeEl)
        ...
        {
            string? error =
                root.TryGetProperty("error", out var errorEl) && errorEl.ValueKind == JsonValueKind.String
                    ? errorEl.GetString()
                    : null;
            throw new InvalidOperationException(
                "GitHub device-flow response was missing a required field" + (error is not null ? $": {error}" : ".")
            );
        }
        ...
        return new DeviceAuthorization(
            ...
            new Uri(verificationUriEl.GetString()),
            TimeSpan.FromSeconds(root.TryGetProperty("interval", out interval) ? interval.GetInt32() : 5),
