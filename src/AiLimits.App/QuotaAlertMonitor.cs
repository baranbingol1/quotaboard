...
internal sealed class QuotaAlertMonitor : IDisposable
{
    ...
    {
        AlertPolicy policy = AlertPolicy.Suggested with
        {
            Enabled = QuotaAlertPreference.LoadEnabled()
        };
...
internal sealed class LocalizedAlertTextProvider : IAlertTextProvider
{
    ...
    public string UsageMessage(double usedPercent, DateTimeOffset? resetsAt) => resetsAt is DateTimeOffset reset
    ...
    public string ResetMessage(TimeSpan remaining) =>
        F("Alert_ResetMessage", FormatRemaining(remaining));
    ...
    private static string ProviderName(string providerId) => providerId.ToLowerInvariant() switch
    {
        "codex" => "Codex",
        "claude" => "Claude Code",
        "opencode" => "OpenCode",
